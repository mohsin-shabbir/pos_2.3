...
class Logs extends Person_controller
{
	...
	function index()
	{
		...
		$data['manage_table']= get_log_manage_table( $this->Customer->get_all_logs( $config['per_page'], $this->uri->segment( $config['uri_segment'] ) ), $this );
		$this->load->view('people/logs',$data);
	}
	...
	*/
	function search()
	{
		...
		$data_rows= get_log_manage_table_data_rows($this->Customer->search_log($search),$this);
		echo $data_rows;
	}
	...
	*/
	function suggest()
	{
		$suggestions = $this->Customer->get_search_log_suggestions($this->input->post('q'),$this->input->post('limit'));
		echo implode("\n",$suggestions);
	}
