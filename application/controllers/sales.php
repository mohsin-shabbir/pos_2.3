...
class Sales extends Secure_area
{
	...
	function _reload($data=array())
	{
		...
		$data['payment_options']=array(
			...
			$this->lang->line('advance_sales_credit') => $this->lang->line('advance_sales_credit') 
		);
	...
	function editSales($sale_id)
	{
		//$data['sales_data'] = 'Mohsin';	
		$this->load->model('sale');
		$result = $this->sale->getReceivedPayments($sale_id);
		$data['payments'] = $result['payments'];
		$data['balance'] =  $result['balance'];	
		$data['sale_id'] = $sale_id;
		$this->load->view('sales/form_sales', $data);
	}
	function addReceivedAmount($sale_id )
	{
		$payment_amount = $this->input->post('amount_received');
		$redirect_uri = $this->input->post('current_url');
		$this->load->model('sale');
		$result = $this->sale->insertPayment($sale_id , $payment_amount );
		$employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
		$this->common_lib->saveLog($result , "Sales" , date('Y-m-d H:i:s') , $employee_id , "");	
